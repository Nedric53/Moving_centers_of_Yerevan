...
from .config import G_LAT, G_LON, PIPELINE_TABLES_DIR, PIPELINE_VECTOR_DIR


def export_heritage_points_and_map(
    yerevan_ll,
    vector_dir=PIPELINE_VECTOR_DIR,
    table_dir=PIPELINE_TABLES_DIR,
    ...
    # ------------------------------------------------------------
    AMEN_POINTS_GPKG = os.path.join(vector_dir, "heritage_points_with_monuments.gpkg")
    AMEN_POINTS_CSV  = os.path.join(table_dir, "heritage_points_with_monuments.csv")
    ...
    if report_html_path:
        ...
        m.save(report_html_path)

        print("Saved map HTML:", report_html_path)
        IFrame(report_html_path, width=1100, height=700)
