...
#    data/build/site/yerevan_precomputed_model.html
# 3) renders both on a comparable METERS scale, centered at historic center (0,0)
#
# Inputs expected:
# - data/reference/city_business_areas/city_business_area_metadata.csv
# - data/reference/city_business_areas/polygons/*.geojson
# - data/build/site/yerevan_precomputed_model.html
#
# Output:
# - data/build/site/city_business_area_comparison.html
...
from .config import (
    CITY_BUSINESS_AREAS_DIR,
    CITY_BUSINESS_AREA_POLYGONS_DIR,
    CITY_COMPARISON_HTML_NAME,
    COMPARE_DIR,
    INTERACTIVE_DIR,
    PRECOMPUTED_MODEL_HTML_NAME,
)

CITY_CSV = CITY_BUSINESS_AREAS_DIR / "city_business_area_metadata.csv"
POLY_DIR = CITY_BUSINESS_AREA_POLYGONS_DIR
YEREVAN_PRECOMP_HTML = INTERACTIVE_DIR / PRECOMPUTED_MODEL_HTML_NAME

OUT_DIR = COMPARE_DIR
OUT_HTML = OUT_DIR / CITY_COMPARISON_HTML_NAME
...
def load_csv_metadata(csv_path: str | Path) -> pd.DataFrame:
...
def load_city_polygons_payload(csv_path: str | Path, poly_dir: str | Path):
    df = load_csv_metadata(csv_path)
    meta = {r["city"]: r for _, r in df.iterrows()}

    files = sorted(glob.glob(str(Path(poly_dir) / "*.geojson")))
    if not files:
...
<head>
  ...
  <style>
    ...
    @media (max-width: 900px) {
      ...
      .desc { font-size: 16px; margin-bottom: 18px; }
      .card {
      ...
      .right { min-width: 0; }
      .legendRow { padding: 12px 16px 8px; gap: 10px; }
      .plotWrap { min-height: 52vh; }
    }
    ...
    @media (max-width: 640px) {
      ...
      .sliderName { margin: 14px 0 8px; }
      .plotWrap { min-height: 320px; }
      .plotLabel { font-size: 12px; }
      .plotLabel.left,
      .plotLabel.right {
        top: auto;
        bottom: 12px;
      ...
      .plotLabel.left { left: 12px; }
      .plotLabel.right { right: 12px; }
      .legendItem { font-size: 12px; }
      .kpi { font-size: 12px; }
    }
  </style>
</head>
...
def main(
    city_csv: str | Path = CITY_CSV,
    poly_dir: str | Path = POLY_DIR,
    yerevan_precomp_html: str | Path = YEREVAN_PRECOMP_HTML,
    out_html: str | Path = OUT_HTML,
):
    city_csv = Path(city_csv)
    poly_dir = Path(poly_dir)
    yerevan_precomp_html = Path(yerevan_precomp_html)
    out_html = Path(out_html)
    out_html.parent.mkdir(parents=True, exist_ok=True)

    if not city_csv.exists():
        raise SystemExit(f"Missing: {city_csv}")
    if not yerevan_precomp_html.exists():
    ...
    ytxt = yerevan_precomp_html.read_text(encoding="utf-8")
